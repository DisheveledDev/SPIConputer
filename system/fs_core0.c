...
/* ASCII case-insensitive compare (names on the card are compared the
 * way FAT looks them up: case is ignored). */
...
/* Case-insensitive filename lookup in a directory. The matching entry's
 * real name goes to the staging buffer; returns its length (0 = not
 * found). */
static int32_t pack_find(const char *dir, const char *name) {
    DIR dj;
    FILINFO fno;
    int32_t found = 0;
    ...
    for (;;) {
        ...
        if (ascii_casecmp(fno.fname, name) == 0) {
            size_t nlen = strlen(fno.fname);
            if (nlen >= RPC_STAGING_SIZE) {
                nlen = RPC_STAGING_SIZE - 1;
            }
            memcpy(rpc_staging(), fno.fname, nlen);
            rpc_staging()[nlen] = '\0';
            found = (int32_t)nlen;
            break;
        }
