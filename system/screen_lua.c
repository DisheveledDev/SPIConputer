...
static int screen_z_order(lua_State *L) {
    ...
    if (!video_set_z_order(v, layer)) {
        lua_pushnil(L);
        lua_pushliteral(L, "z-order is only available for text modes 0-3 and layers 0-2");
        return 2;
    }
...
static int screen_clear(lua_State *L) {
    ...
    if (v->mode == VIDEO_MODE_PIXEL) {
        ...
        size_t cells = (size_t)video_char_cols(v) * video_char_rows(v);
