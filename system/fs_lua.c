 ...
 *   fs.find(name [, path])   -> real entry name (case-insensitive) or nil
 ...
 * so a card that carries both runs the compiled program; a `*.prg`
 * path is always read exactly as given. */
FRESULT fs_lua_read_program(const char *path, char **out, size_t *out_len,
                            char *resolved, size_t resolved_size) {
    size_t len = strlen(path);
    if (len > 4 && len - 4 < FS_LUA_PATH_MAX &&
        strcmp(path + len - 4, ".lua") == 0) {
        char alt[FS_LUA_PATH_MAX];
        snprintf(alt, sizeof(alt), "%.*s.prg", (int)(len - 4), path);
        FRESULT fr = fs_lua_readall(alt, out, out_len);
        if (fr == FR_OK) {
            if (resolved) {
    ...
    FRESULT fr = fs_lua_readall(path, out, out_len);
    if (fr == FR_OK && resolved) {
...
/* Case-insensitive lookup of `name` in `path` (default the root): the
...
static int fs_find(lua_State *L) {
    ...
    lua_pushlstring(L, (const char *)rpc_staging(), (size_t)resp.value);
    return 1;
}
...
static int loadfile_from_sd(lua_State *L, const char *path) {
    ...
    FRESULT err = fs_lua_read_program(path, &buf, &len, resolved,
                                      sizeof(resolved));
    if (err != FR_OK) {
        lua_pushnil(L);
    ...
    int st = luaL_loadbufferx(L, buf, len, resolved[0] ? resolved : path, "bt");
    free(buf);
    if (st != LUA_OK) {
        lua_pushnil(L);
        lua_insert(L, -2);
...
/* Searcher for require(): looks for NAME, NAME.prg, NAME.lua, /NAME,
 * /NAME.prg, /NAME.lua, /lib/NAME(.prg/.lua) on the SD card. Returns a
 * loader function or an error message (exactly one result, as the
 * require() protocol expects). Compiled `.prg` modules win over their
 * source siblings. */
static int searcher_from_sd(lua_State *L) {
    ...
    const char *fmts[] = {"%s",       "%s.prg",       "%s.lua",
                          "/%s",      "/%s.prg",      "/%s.lua",
                          "/lib/%s",  "/lib/%s.prg",  "/lib/%s.lua"};
