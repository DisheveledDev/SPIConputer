...
/* Foreground launch of a Lua source string (ExecuteString). `name` is
 * used in error messages; the arguments follow the same rules as
 * program_launch_args(). */
...
/* Retrieve and clear a completed noninteractive utility result. */
 ...
 * A `*.lua` path prefers its compiled `*.prg` sibling; `resolved`
 * (optional) receives the path actually read. Returns 0 on success;
 * on failure *err gets a static message. */
