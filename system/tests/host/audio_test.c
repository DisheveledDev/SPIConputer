...
static const char *MML_LUA =
    ...
    "  local function events(ch)\n"
    "    local list = {}\n"
    "    for i = 1, #ch, 10 do\n"
    "      local at, sound, note, dur, vol, pan = string.unpack('<I4BBI2Bb', ch, i)\n"
    "      list[#list + 1] = { at = at, sound = sound, note = note, dur = dur, vol = vol, pan = pan }\n"
    "    end\n"
    "    return list\n"
    "  end\n"
    "  local ch = events(score.channels[1])\n"
    "  local parts = {}\n"
    "  for _, e in ipairs(ch) do parts[#parts + 1] = e.at .. '/' .. e.note .. '/' .. e.dur .. '/' .. e.vol end\n"
    "  results.ch1 = table.concat(parts, ' ')\n"
    "  local ch2 = events(score.channels[2])\n"
    "  results.ch2 = #ch2 .. ':' .. ch2[2].at .. ':' .. ch2[1].note\n"
