...
/* Root directory view: the boot script, the root-level mock files, the
 * "lib" directory and boot.log. Entries are produced one per call
 * (dp->idx counts them), so tests can add files with mock_set_file()
 * and fs.ls()/fs.find() see them. */
FRESULT f_readdir(DIR *dp, FILINFO *fno) {
    if (g_ejected) return FR_NOT_READY;
    int index = dp->idx;
    memset(fno, 0, sizeof(*fno));

    bool has_os_lua = false;
    for (int i = 0; i < MOCK_FILES_MAX; i++) {
        if (g_files[i].path && strcmp(g_files[i].path, "os.lua") == 0) {
            has_os_lua = true;
        }
    }
    if (!has_os_lua) {
        if (index == 0) {
            strcpy(fno->fname, "os.lua");
            fno->fsize = (FSIZE_t)g_os_lua_len;
            dp->idx++;
            return FR_OK;
        }
        index--;
    ...
    for (int i = 0; i < MOCK_FILES_MAX; i++) {
        if (!g_files[i].path || strchr(g_files[i].path, '/')) {
            continue;
        }
        if (index == 0) {
            snprintf(fno->fname, sizeof(fno->fname), "%s", g_files[i].path);
            fno->fsize = (FSIZE_t)g_files[i].len;
            dp->idx++;
            return FR_OK;
        }
        index--;
    ...
    index--;
    if (index == 0) {
        strcpy(fno->fname, "boot.log");
        fno->fsize = (FSIZE_t)g_boot_log_len;
        dp->idx++;
        return FR_OK;
    }
    return FR_OK; /* end of directory: fname[0] stays 0 */
}
