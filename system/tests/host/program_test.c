...
/* ---------------- test 9: Execute / ExecuteString / args ---------------- */

static const char *X_PARENT_LUA =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "local phase = 0\n"
    "function tick()\n"
    "  phase = phase + 1\n"
    "  if phase == 1 then\n"
    "    local ok, err = Execute('child.lua', 'one', 'two words')\n"
    "    log('exec:' .. tostring(ok) .. ':' .. tostring(err) .. '\\n')\n"
    "  elseif phase == 2 then\n"
    "    log('resumed1\\n')\n"
    "    local source = fs.readall('inner.lua')\n"
    "    local ok, err = ExecuteString(source, 'inner')\n"
    "    log('execstring:' .. tostring(ok) .. ':' .. tostring(err) .. '\\n')\n"
    "  else\n"
    "    log('resumed2\\n')\n"
    "    ExitProgram()\n"
    "  end\n"
    "end\n"
    "function finish() log('finish\\n') end\n";

static const char *X_CHILD_LUA =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup()\n"
    "  log('child:' .. #args .. ':' .. args[1] .. ':' .. args[2] .. ':' ..\n"
    "    tostring(args[3]) .. '\\n')\n"
    "  ExitProgram()\n"
    "end\n"
    "function tick() end\n";

static const char *X_INNER_LUA =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup()\n"
    "  log('inner:' .. #args .. ':' .. tostring(args[1]) .. '\\n')\n"
    "  ExitProgram()\n"
    "end\n"
    "function tick() end\n";

static void test_execute(void) {
    mock_set_file("parent.lua", X_PARENT_LUA);
    mock_set_file("child.lua", X_CHILD_LUA);
    mock_set_file("inner.lua", X_INNER_LUA);
    boot("parent.lua");

    for (int i = 0; i < 12 && program_top() != NULL; i++) {
        program_scheduler_step();
    }
    CHECK(program_top() == NULL, "execute chain finished");
    expect_log("child:2:one:two words:nil\n"
               "exec:true:nil\n"
               "resumed1\n"
               "inner:1:inner\n"
               "execstring:true:nil\n"
 ...
 * mock card as `path` (the format `luac` / the IDE produce). */
typedef struct {
    char *data;
    size_t len;
    size_t cap;
...
static int dump_to_buffer(lua_State *L, const void *p, size_t size, void *ud) {
    (void)L;
    dump_buffer_t *b = (dump_buffer_t *)ud;
    if (b->len + size > b->cap) {
...
static void mock_set_compiled(const char *path, const char *source) {
    lua_State *L = luaL_newstate();
    if (!L ||
        luaL_loadbufferx(L, source, strlen(source), path, "t") != LUA_OK) {
        printf("FAIL: compile %s: %s\n", path,
               L ? lua_tostring(L, -1) : "no state");
        g_failures++;
        if (L) {
            lua_close(L);
        }
    ...
    dump_buffer_t buf = {0};
    if (lua_dump(L, dump_to_buffer, &buf, 0) != 0 || buf.failed) {
        printf("FAIL: dump %s\n", path);
        g_failures++;
    } else {
        mock_set_file_bytes(path, buf.data, buf.len);
    }
    free(buf.data);
    lua_close(L);
}

static const char *PRG_BIN =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup() log('bin-setup\\n') end\n"
    "function tick() log('bin-tick\\n') ExitProgram() end\n";

static const char *PRG_SRC =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup() log('src-setup\\n') end\n"
    "function tick() log('src-tick\\n') ExitProgram() end\n";

static const char *PRG_LOADER =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup()\n"
    "  local ok, err = pcall(dofile, 'loaded.prg')\n"
    "  log('dofile:' .. tostring(ok) .. ':' .. tostring(err) .. '\\n')\n"
    "  local ok2, mod = pcall(require, 'libmod')\n"
    "  log('require:' .. tostring(ok2) .. ':' .. tostring(mod) .. '\\n')\n"
    "  ExitProgram()\n"
    "end\n"
    "function tick() end\n";

static const char *PRG_LOADED =
    "local f = fs.open('boot.log','a') f:write('loaded-chunk\\n') f:close()\n"
    "return 'loaded'\n";

static const char *PRG_MODULE = "return 'lib-ok'\n";

static const char *UTILITY_LUA =
    "__spi_interactive = false\n"
    "function setup() UtilityResult(true, '1 file(s) copied') end\n"
    "function tick() end\n";

static const char *UTILITY_PARENT_LUA =
    "local function log(m) local f = fs.open('boot.log','a') f:write(m) f:close() end\n"
    "function setup() Launch('utility.lua') end\n"
    "function tick()\n"
    "  local ok, message = UtilityPoll()\n"
...
static void test_compiled_programs(void) {
    ...
    boot("only.prg");
    program_scheduler_step();
    CHECK(program_top() == NULL, "compiled program exits");
    expect_log("bin-setup\nbin-tick\n");

    /* 2. The compiled .prg wins over its .lua sibling. */
    mock_set_file("both.lua", PRG_SRC);
    mock_set_compiled("both.prg", PRG_BIN);
    boot("both.lua");
    program_scheduler_step();
    expect_log("bin-setup\nbin-tick\n");

    /* 3. A .lua request falls back to a .prg-only program. */
    mock_set_compiled("gone.prg", PRG_BIN);
    boot("gone.lua");
    program_scheduler_step();
    expect_log("bin-setup\nbin-tick\n");

    /* 4. dofile()/require() load compiled chunks too. */
    mock_set_file("loader.lua", PRG_LOADER);
    mock_set_compiled("loaded.prg", PRG_LOADED);
    mock_set_compiled("libmod.prg", PRG_MODULE);
    boot("loader.lua");
    program_scheduler_step();
    CHECK(program_top() == NULL, "loader exits");
