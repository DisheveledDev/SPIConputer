...
| Process model | `program.c` — 4-program stack, per-program Lua state (96 KB heap cap, `PROGRAM_HEAP_CAP`), optional heap-allocated audio state, timers, per-program event rings; `Launch(..., replace)` hands the stack over and releases the replaced state; noninteractive utilities keep their isolated Lua state but return `UtilityResult` text to the parent via `UtilityPoll` (an interactive program may end with a result too; the parent rebuilds a table result with a direct reader in `sys_lua.c`, not a Lua compile, and keys typed while a utility runs go to the interactive program below it); `sys_lua.c` exposes TimeNow/Pid/ExitProgram/Launch/Execute/ExecuteString/UtilityResult/UtilityPoll/TimerCreate/TimerStop/InputPoll/InputControl/WaitVSync/Compile (Compile builds a `.prg` from a `.lua` on the card in a scratch `lua_State` on the system heap, same output as the IDE; the shell's `COMPILE` command wraps it) |
| Shell / card programs | **Not in this repo.** `core/boot.prg`, the shell (`core/os.prg`), the APPS launcher (`core/apps.prg`), apps, utilities and games are SPIEdit projects developed outside the OS source tree (this workspace keeps them in `software/`; each builds into its own `build/`, and `software/install.sh` assembles a card image in `software/sdcard/`, git-ignored). Card layout: `core/` (system, raw programs), `apps/` (`name.app`), `utils/` (`name.util`: commands that run once with `args` and return a table via `UtilityResult`), `games/` (`name.game`: launched with `Launch(..., replace)`, so the shell is freed; the device reboots when the last program exits, see `lua_main.c`), `data/` (user files, the only area the shell's file commands touch). The shell resolves a command name to built-ins, aliases, `utils/`, `apps/`, `games/`, then loose programs. It is resident under every program, so it keeps only what needs its state (CD, TYPE with the pager, EXEC/autoexec, ALIAS, HISTORY, COLOR, MODE, MEM, RUN, APPS, RESET, line editing with history and TAB completion, `>`/`>>` redirection, wildcard expansion) and is built with `strip_debug`; the DOS-style external commands are utilities (DIR, COPY, DEL, REN, MD, RD, TOUCH, STAT, COMPILE, HELP/UTILS, plus wc, grep, find, tree, hexdump, head, tail, sort, calc, sysinfo). Heap after start-up (host, stripped): about 80 KB of the 96 KB cap, so new shell features belong in utilities. It has no exit command beyond RESET: the shell is the OS. The OS only provides the runtime, `lua.md` the contract |
