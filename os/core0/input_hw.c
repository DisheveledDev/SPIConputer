 ...
 * the C64 key matrix (product board), polls both joysticks (product
 * board), and drains RS232 RX bytes into the `input=` line parser. All
 * queue pushes happen here (IRQ context), keeping the SPSC queue's
 * single-producer guarantee. The main loop only feeds the watchdog.
 *
 * Dev board (RP2040): only the RS232 path is compiled in; the keyboard
 * and joysticks come over the serial link.
 */
#include <stdint.h>
...
static bool input_tick_1khz(repeating_timer_t *rt) {
    ...
    /* RS232 keyboard-in. 1 kHz draining is ~10x the worst-case input
     * rate at 115200 baud (12 bytes/ms); the 32-byte RX FIFO absorbs
     * bursts between ticks. */
    while (uart_is_readable(RS232_UART_INSTANCE)) {
        input_rs232_feed((uint8_t)uart_getc(RS232_UART_INSTANCE));
    }
    return true;
}
