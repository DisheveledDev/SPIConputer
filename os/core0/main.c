...
int main(void)
{
    ...
    // Input subsystem (Phase 3): 1 kHz matrix/joystick scan + RS232 RX
    // draining, all producing events on core 0.
    input_hw_init();

    // Serial mirror (Phase 1): streams the tile layer to the terminal
    // app over RS232 (modes 0/1).
    serial_mirror_init();
    ...
    for (;;) {
        ...
        if (time_us_64() - boot_us < 5000000) {
            progress = true; /* boot grace: cover SD init + os.lua */
        }
