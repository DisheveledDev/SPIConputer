...
static program_t *program_create(const char *path, const char *arg,
                              const char **err) {
    ...
    char *buf = NULL;
    size_t len = 0;
    if (program_read_file(path, &buf, &len, err) != 0) {
        lua_close(p->L);
        pool_free(p);
        return NULL;
    }
    int st = luaL_loadbufferx(p->L, buf, len, path, "t");
    free(buf);
    if (st != LUA_OK) {
     ...
     * the body defines setup()/tick()/finish(). The launch argument
     * (if any) arrives as the chunk's first vararg. */
    p->chunk_ref = luaL_ref(p->L, LUA_REGISTRYINDEX);
    lua_rawgeti(p->L, LUA_REGISTRYINDEX, p->chunk_ref);
    if (arg) {
        lua_pushstring(p->L, arg);
    }
    st = lua_pcall(p->L, arg ? 1 : 0, 0, 0);
    if (st != LUA_OK) {
...
static void program_pop(program_t *p) {
    ...
    g_current_video = s_top ? s_top->video : NULL;
    g_current_audio = s_top ? s_top->audio : NULL;
}
...
int program_launch(const char *path, const char *arg, const char **err) {
    *err = NULL;
    if (s_top && s_top->video->mode == VIDEO_MODE_PIXEL) {
    ...
    program_t *p = program_create(path, arg, err);
    if (!p) {
        if (s_top) {
            program_resume(s_top);
        }
