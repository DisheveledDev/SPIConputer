...
- **Lock-free where possible, brief spinlocks otherwise.** Video map
  ...
  completion).
- **Lua integration follows the existing `fatfs_lua.c` style:** one C file
...
- **Host-testable logic.** Anything not touching registers (video state
  machine, renderer, Lua bridge, RPC protocol) gets a mock header and a
  host test in `tests/host/`.
...
Tasks:
1. Vendor `pico_scanvideo_dpi` (pico-extras) into the RP2354B build only.
2. Fixed timing: 640x480@60, 25.175 MHz pixel clock, one configuration for
   the life of the process. No mode-switch resync ever.
...
6. Lua API module `screen_lua.c`:
   ...
   - `ScreenClear([char])`
7. Serial mirror (all boards, always compiled on dev board): core 0 builds
   a 1201-byte frame (40x30 chars + NUL) from the active char map and
   streams it over the RS232 UART, repeated (~9.6 fps). Only for modes 0/1.
...
- On product board (Phase 6 hardware): 640x480 image on a monitor for all
...
## Phase 2 — Input Subsystem (core 0)
...
4. RS232 keyboard-in: UART RX on core 0 (IRQ or polled), bytes → KEY
   events (pressed + released synthesis for the host app's convenience).
...
## Phase 3 — Dual-Core Runtime and SD RPC
...
Tasks:
1. `fs.readall` / `fs.writeall` over RPC (Phase 3 already adds them).
...
## Phase 6 — Product Board Bring-Up (RP2354B)
...
| Video snapshot pool (4 × ~30 KB) | 120 KB |
| Lua states: 4 programs × (96 KB heap + 16 KB stack + VM) | ~448 KB worst case |
...
| 9.6 fps serial mirror too slow for editor UX | Editor only redraws dirty cells; or add a delta protocol later (backwards-compatible frame type byte) |
| Lua heap fragmentation across long sessions | Bounded per-program allocators + program exit frees everything |
| HSTX + SPI1 coexistence bugs on real silicon | Phase 6 burn-in; SD access and video run simultaneously from day one in tests |
