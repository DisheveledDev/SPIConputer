...
-- Built 2026-09-11T22:09:57Z from components:
--   snippet header
--   lua     document
--   lua     screen
--   lua     edits
...
local menu_defs = {
    ...
    {name = "OPTIONS", items = {"Toggle cursor", "Clear menu"}},
...
local function draw_string(x, y, text, attr)
    for i = 1, #text do
        if x + i - 1 < W and y >= 0 and y < 30 then
...
local function draw_menu_layer()
    ...
    for i, menu in ipairs(menu_defs) do
        local x = menu_x(i)
        local attr = i == menu_top and 0x80 or 0x02
    ...
    if menu_open then
        ...
        fill_rect(x, 1, width, #menu.items, 0x02)
        for i, item in ipairs(menu.items) do
            local attr = i == menu_item and 0x80 or 0x02
...
local function draw_dialog_layer()
    ...
    if dialog_kind == "goto" then
        fill_rect(7, 10, 26, 5, 0x02)
        draw_string(9, 11, "GO TO LINE", 0x02)
        draw_string(9, 13, dialog_text, 0x80)
        ScreenOut(9 + #dialog_text, 13, 32, 0x80)
        draw_string(9, 14, "ENTER ACCEPT  ESC CANCEL", 0x02)
    elseif dialog_kind == "help" then
        fill_rect(2, 4, 36, 21, 0x02)
        draw_string(15, 5, "EDITOR HELP", 0x80)
        local help = {
            "F10 or ALT+F  FILE MENU",
            "F2 or ALT+E  EDIT MENU",
            "F3 or ALT+O  OPTIONS MENU",
            "F1 or ALT+H  HELP MENU",
        ...
        for i, line in ipairs(help) do
            draw_string(4, 7 + i, line, 0x02)
        end
        draw_string(8, 20, "PRESS ESC TO CLOSE", 0x80)
    end
end
